...
{
    public class FaucetFreeLTCFunService2 : IFaucetFreeLTCFunService2
    {
            ...
            ILogger<FaucetFreeLTCFunService> logger)
        {
            ...
            this.cFWebProxyService = cFWebProxyService;
            this.logger = logger;
        }
        ...
        public async Task<FreeLTCFunLoginPageResponse> DoLogin(string hostName, string email)
        {
            try
            {
                ...
                var response = await GetResponse(email, hostName, FaucetRouteHelper.LoginPost(host), HttpMethod.Post, body, "form",follow:false);
                var location = response.Headers.Location;
                var doc = await response.ToHtml();

                if (location != null)
                {
                    var response1 = await GetResponse(email, hostName, location.AbsoluteUri, HttpMethod.Post, body, "form", follow: false);
                    doc = await response1.ToHtml();
                }
        ...
        //string email, string hostName, string path, HttpMethod httpMethod,
        //object body = null!, string typeBody = "", string xIconCapthca = "", bool follow = true)
        //    {
        //        var host = await faucetHost.GetByName(hostName);
        //        if (host == null || !host.IsActive) throw new Exception("Host not active");
        ...
        //        var targetUrl = host.BaseURL!.TrimEnd('/') + "/" + path.TrimStart('/');
        ...
        //        }
        //        if (!string.IsNullOrWhiteSpace(xIconCapthca))
        //        {
        //            headers["X-IconCaptcha-Token"] = xIconCapthca;
        //        }

        //        var response = await cFWebProxyService.SendAsync(
        //            targetUrl, httpMethod, body, typeBody, headers, follow);
        ...
        private async Task<HttpResponseMessage> GetResponse(string email, string hostName, string path, HttpMethod httpMethod,
    object body = null!, string typeBody = "", string xIconCaptcha = "", bool follow = true)
        {
            try
            {
                var host = await faucetHost.GetByName(hostName);
                ...
                // ✅ Use proxy worker instead of direct host
                string workerUrl = "https://proxyworker.sistalani.workers.dev/";
                var httpClient = HttpHelper.GetHttpClientCF(user, workerUrl, follow);

                // Encode target URL
                string targetUrl = $"{host.BaseURL}{path}";
                string proxyUrl = $"{workerUrl}?url={Uri.EscapeDataString(targetUrl)}";

                var request = new HttpRequestMessage(httpMethod, proxyUrl);
                if (!string.IsNullOrWhiteSpace(xIconCaptcha))
                    request.Headers.Add("X-IconCaptcha-Token", xIconCaptcha);
        ...
        private async Task<HttpResponseMessage> GetResponseNormal(string email, string hostName, string path, HttpMethod httpMethod,
            object body = null!, string typeBody = "", string xIconCapthca = "", bool follow = true)
        {
            try
            {
